...
enrollment AS (
    SELECT 
        ...
        e.is_boarding,
        e.housing_type
    FROM {{ source('main', 'fct_student_term_enrollment') }} e
    JOIN {{ source('main', 'dim_terms') }} t ON e.term_key = t.term_key
    WHERE t.term_name LIKE 'Fall%'
),

attendance AS (
    SELECT * FROM {{ ref('int_student_attendance') }}
),

behavior AS (
    SELECT * FROM {{ ref('int_student_behavior') }}
),

courses AS (
    SELECT * FROM {{ ref('int_student_course_performance') }}
),

assignments AS (
    SELECT * FROM {{ ref('int_student_assignments') }}
)
